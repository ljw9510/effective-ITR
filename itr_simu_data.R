...
make_data_binary <- function(n, p = 20, case = 1, sigma = 1) {
  ...
  sigma_x <- err_fn(x, trt, case)
  err <- rnorm(n, 0, sd = sigma) * sqrt(sigma_x)
  p_ftn <- 1/(1+exp(-(eff_main + eff_interaction + err)))
  y <- ifelse(p_ftn >= 0.5, 1, 0) # to make potential outcome be equal to observation, no randomness from binary
  
  eff_cate <- counterfactuals_fn(x, case) 
  opt_trt <- max.col(eff_cate)
  potention_p <- 1/(1+exp(-(eff_main + eff_cate + err)))
  potential_outcome <- ifelse(potention_p >= 0.5, 1, 0) 
